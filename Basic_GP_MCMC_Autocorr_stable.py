...
def walker_diagnostic_plot(samples,lnP,ndim):
    ...
    labels=['Mean',r'$\log A$',r'$\Gamma_1$',r'$\log P$',r'$\log m$']
    for i in range(ndim):
...
def uniform_lnprior(parvec):
    men,logA,gamma, logP, logm = parvec
...
if __name__ == '__main__':
    ...
    input_pars = [mean,amp,gamma,logPeriod,metric]

    k = input_pars[1] * george.kernels.ExpSine2Kernel(gamma=input_pars[2], log_period=input_pars[3]) * george.kernels.ExpSquaredKernel(input_pars[4])
    gp = george.GP(k,mean = input_pars[0], fit_mean =True)

    input_pars = gp.get_parameter_vector()
